<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Global variables -->
    <xsl:variable name="varResourceID"
        select="/ead:ead/ead:archdesc[@level='collection']/ead:did/ead:unitid"/>
    ...
    <xsl:template match="ead:c[@level='file'] | ead:c[@level='item']">
        ...
        <xsl:choose>
            ...
            <xsl:when test="ead:did/ead:unitid and not(ead:dao)">
                        ...
                        <xsl:when test="position()=2">
                            <xsl:choose>
                                <xsl:when test="contains(.,'-')">
                                    <xsl:value-of
                                        select="concat('; ',@type, ' ', format-number(substring-before(.,'-'),'000'))"/>
                                    <xsl:value-of
                                        select="concat('-', format-number(substring-after(.,'-'),'000'))"
                                    />
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of
                                        select="concat('; ', @type, ' ', format-number(.,'000'))"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:when>
                        <xsl:otherwise/>
                ...
                <xsl:value-of select="concat('http://hdl.handle.net/2345.2/',ead:did/ead:unitid)"/>
                <xsl:value-of select="$varTab"/>
                <!-- isComponent -->
                ...
                <xsl:choose>
                    <xsl:when
                        test="contains(translate(ead:did/ead:unitdate,'UNDATED','undated'),'undated')">
                        <xsl:choose>
                            <xsl:when
                                test="(ancestor::ead:c/ead:did/ead:unitdate) and (ancestor::ead:c/ead:did/ead:unitdate !='undated')">
                                <xsl:value-of
                                    select="substring-before((ancestor::ead:c/ead:did/ead:unitdate[@normal!='']/@normal)[last()],'/')"/>
                                <xsl:value-of select="$varTab"/>
                                <xsl:value-of
                                    select="substring-after((ancestor::ead:c/ead:did/ead:unitdate[@normal!='']/@normal)[last()],'/')"/>
                                <xsl:value-of select="$varTab"/>
                                <xsl:value-of
                                    select="translate(normalize-space(ead:did/ead:unitdate),'UNDATED','undated')"/>
                                <xsl:value-of select="$varTab"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of
                                    select="substring-before(//ead:ead/ead:archdesc/ead:did/ead:unitdate[@type='inclusive']/@normal,'/')"/>
                                <xsl:value-of select="$varTab"/>
                                <xsl:value-of
                                    select="substring-after(//ead:ead/ead:archdesc/ead:did/ead:unitdate[@type='inclusive']/@normal,'/')"/>
                                <xsl:value-of select="$varTab"/>
                                <xsl:value-of
                                    select="translate(normalize-space(ead:did/ead:unitdate),'UNDATED','undated')"/>
                                <xsl:value-of select="$varTab"/>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of select="substring-before(ead:did/ead:unitdate/@normal,'/')"/>
                                <xsl:if test="(substring-before(ead:did/ead:unitdate/@normal,'/'))!=(substring-after(ead:did/ead:unitdate/@normal,'/'))">
                                    <xsl:text>-</xsl:text>
                                    <xsl:value-of select="substring-after(ead:did/ead:unitdate/@normal,'/')"/>
                                </xsl:if>
                                <xsl:value-of select="$varTab"/>
                ...
                <xsl:variable name="varLabel">
                    <xsl:value-of select="normalize-space(ead:did/ead:container/@label)"/>
                </xsl:variable>
                <xsl:value-of
                ...
                <!-- objectType -->
                <xsl:value-of
                    select="$varInstanceLookup/atDAOInstanceTypeLookup/atDAOInstance[@type=$varLabel]/@mods"/>
                <xsl:value-of select="$varTab"/>
